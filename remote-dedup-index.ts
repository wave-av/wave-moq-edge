...
export interface DedupService {
  ...
  addRef(org: string, refKey: string, hash: string): Promise<AddRefResult>;
  release(org: string, hash: string): Promise<ReleaseResult>;
  lookup(org: string, hash: string): Promise<IndexRow | null>;
  lookupRef(org: string, refKey: string): Promise<string | null>;
  refCountForHash(org: string, hash: string): Promise<number>;
}
...
export function makeRemoteDedupIndex(svc: DedupService): DedupIndex {
  return {
    ...
    addRef: (org, refKey, hash) => svc.addRef(org, refKey, hash),
    release: (org, hash) => svc.release(org, hash),
    lookup: (org, hash) => svc.lookup(org, hash),
    lookupRef: (org, refKey) => svc.lookupRef(org, refKey),
    refCountForHash: (org, hash) => svc.refCountForHash(org, hash),
  };
}
