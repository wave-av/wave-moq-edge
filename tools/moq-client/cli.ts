...
async function cmdSession(args: Args, role: 'subscribe' | 'publish'): Promise<number> {
  ...
  try {
    transport = await openTransport(url, args.flags.get('transport') ?? 'websocket', role === 'publish' ? 'publish' : 'subscribe');
  } catch (e) {
...
async function cmdPublishSubgroup(args: Args): Promise<number> {
  ...
  let transport: Transport;
  try {
    transport = await openTransport(url, args.flags.get('transport') ?? 'websocket', 'publish');
  } catch (e) {
    const report = {
      peer: redact(url),
      outcome: 'connection-refused',
