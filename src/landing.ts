...
<p class="lead">moq-edge moves live media sub-second across a global edge network. Publish a track, subscribe to it, and every viewer sees the moment while it's still the moment. Live now on IETF MoQ draft-18. Apache-2.0 open source.</p>
...
<div class="btns">
  <a class="btn primary" href="https://console.wave.online">Sign up →</a>
  <a class="btn ghost" href="https://github.com/wave-av/wave-moq-edge#quick-start">Publish your first track →</a>
</div>
</div>

<h2>No CDN to build. No HLS to stitch.</h2>
<pre class="box">curl -X POST https://moq.wave.online/v1/publish/acme/cam1
→ WebTransport URL to publish to

curl https://moq.wave.online/v1/subscribe/acme/cam1
→ WebTransport URL to subscribe from</pre>
<p class="sub">It's live right now at moq.wave.online on IETF MoQ <span class="acc">draft-18</span>, the current working draft, not a someday. One Durable Object per track is the rendezvous. Publishers write to it, and the edge fans each object out to every subscriber. You bring the encoder. WAVE moves the frames sub-second. <span class="good">Apache-2.0 open source. Read every byte.</span></p>
...
<p class="sub" style="margin-top:1.4rem">Open source under Apache-2.0, with an express patent grant. Canonical source at
...
export function landingPage(): string {
  return shell({
    ...
    description: 'moq-edge moves live media sub-second across a global edge network. Publish a track, subscribe to it, and every viewer sees the moment while it\'s still the moment. Live now on IETF MoQ draft-18. Apache-2.0 open source.',
    ...
    cta: {
      ...
      primaryHref: 'https://console.wave.online',
